...
public abstract class UserData extends PlayerExtension implements IConf {
	...
	public final void reloadConfig() {
		...
		ignoreAfk = _getIgnoreAfk();
		ignoreJoin = _getIgnoreJoin();
		ignoreLeave = _getIgnoreLeave();
	...
	private BigDecimal _getMoney() {
		...
		// NPC banks are not actual player banks, as such they do not have player
		// starting balance.
		if (isNPC()) {
	...
	private void saveCommandCooldowns() {
		...
		//
		// Serialization is done as a map list instead of a config section due to
		// limitations.
		// When serializing patterns (which commonly include full stops .)
		// Bukkit/Essentials config framework
		// interprets it as a path separator, thus it breaks up the regex into sub nodes
	...
	public boolean isPromptingClearConfirm() {
		return confirmClear != null ? confirmClear
				: ess.getSettings().isConfirmCommandEnabledByDefault("clearinventory");
	}
	...
	private boolean _getIgnoreAfk() {
		return config.getBoolean("ignore-afk", false);
	}

	public boolean isIgnoreAfkEnabled() {
		return ignoreAfk;
	}

	public void setIgnoredAfkEnabled(boolean set) {
		ignoreAfk = set;
		config.setProperty("ignore-afk", set);
		config.save();
	}

	private boolean ignoreJoin;

	private boolean _getIgnoreJoin() {
		return config.getBoolean("ignore-join", false);
	}

	public boolean isIgnoreJoinEnabled() {
		return ignoreJoin;
	}

	public void setIgnoredJoinEnabled(boolean set) {
		ignoreJoin = set;
		config.setProperty("ignore-join", set);
		config.save();
	}

	private boolean ignoreLeave;

	private boolean _getIgnoreLeave() {
		return config.getBoolean("ignore-leave", false);
	}

	public boolean isIgnoreLeaveEnabled() {
		return ignoreLeave;
	}

	public void setIgnoredLeaveEnabled(boolean set) {
		ignoreLeave = set;
		config.setProperty("ignore-leave", set);
		config.save();
	}
